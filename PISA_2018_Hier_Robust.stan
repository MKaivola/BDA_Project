...
model {
  // Hyperpriors
  mu_alpha ~ student_t(3, alpha_mu_prior, alpha_sd_prior);
  tau_alpha ~ student_t(3, 0, alpha_tau_scale);

  for (j in 1:P) {
    mu_j[j] ~ student_t(3, beta_mu_prior[j], beta_sigma_prior[j]);
    tau_j[j] ~ student_t(3, 0, beta_tau_scale);
  }
  
  // Priors
  sigma ~ student_t(3, 0, sigma_sd_prior);
  nu ~ gamma(2,0.1);
  ...
  // Likelihood
  for (i in 1:2) {
    y_std[,i] ~ student_t(nu, mu_std[, i], sigma);
  }
}
...
generated quantities {
  ...
  for (i in 1:2) {
    for (j in 1:N) {
      log_lik[j,i] = student_t_lpdf(y_std[j,i] | nu, mu_std[j,i], sigma);
      y_rep[j,i] = student_t_rng(nu, mu_std[j,i] * y_sd[i] + y_mean[i], sigma * y_sd[i]);
    }
  }
}
